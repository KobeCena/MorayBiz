...
#import "DetailViewControllerCategory.h"
#import "sqlite3.h"
#import <MessageUI/MFMailComposeViewController.h>


@implementation DetailViewControllerCategory

@synthesize detailTextView = _detailTextView;
@synthesize imageView;
@synthesize detailItem = _detailItem;
@synthesize detailDescriptionLabel, navItem;
...
- (void)viewDidLoad
{
    ...
    {
        [self refresh];
        [[NSUserDefaults standardUserDefaults] addObserver:self forKeyPath:@"CatChoiceDetail" options:(NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld) context:nil];
    }
    ...
    if (BusName == nil)
    {
        if ([def objectForKey:@"CategoryChoiceDetail"] == nil)
        {
            self.detailTextView.text = @"Please file a bug report from the AppStore if you see this error.";
        }
        else
        {
            self.detailTextView.text = @"Please select a Business.";
        }
    }
    else 
    {
    ...
    [navItem setTitle:[def objectForKey:@"CatChoiceDetail"]];
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)
    {
        [self loadMap];
    }
...
{
    def = [NSUserDefaults standardUserDefaults];
    NSURL *finalURL = [[NSURL alloc] initWithString:[def objectForKey:@"finalURLCatPad"]];
...
- (void)refresh
{
    ...
    [def setObject:fullString forKey:@"finalURLCatPad"];
}
...
- (IBAction)searchPressed:(id)sender {
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)
    {
        ...
        {
            [self performSegueWithIdentifier:@"popoverCat" sender:sender];
        }
    }
}

- (IBAction)detailsPressed:(id)sender {
    [self performSegueWithIdentifier:@"detailsPressedCat" sender:self];
}
