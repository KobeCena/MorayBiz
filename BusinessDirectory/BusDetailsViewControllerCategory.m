...
#import "BusDetailsViewControllerCategory.h"
#import "sqlite3.h"
#import <MessageUI/MFMailComposeViewController.h>

@interface BusDetailsViewControllerCategory ()

@end

@implementation BusDetailsViewControllerCategory
@synthesize navItem;
    ...
    if (sqlite3_open(dbpath, &db) == SQLITE_OK)
    {
        ...
        if (sqlite3_prepare_v2(db, query_stmt, -1, &statement, NULL) == SQLITE_OK)
        {
            if (sqlite3_step(statement) == SQLITE_ROW)
            {
               ...
               // busAddr3 = [[NSString alloc] initWithUTF8String:(const char *) sqlite3_column_text(statement, 5)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (selected == webURL)
    {
        [def setObject:selected forKey:@"webStringCat"];
        [self performSegueWithIdentifier:@"webModalCat" sender:self];
    }
    ...
    if (selected == fullBusAddr)
    {
       ...
       // [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[googleCall stringByAppendingString:plusString]]];
        ...
        def = [NSUserDefaults standardUserDefaults];
        [def setObject:finalString forKey:@"finalStringCat"];
        [self performSegueWithIdentifier:@"mapsModalCat" sender:self];
    }
